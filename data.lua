...
function data.jobName(jobId)
    if jobId == nil or jobId < 1 or jobId > data.maxJobId then
        return data.unknownJob;
    end
    return data.jobs[jobId] or data.unknownJob;
end

function data.zoneName(zoneId)
    if zoneId == nil or data.zones[zoneId] == nil then
        return data.unknownZone;
    end
    return data.zones[zoneId];
end

function data.buildMember(name, zoneId, mainJobId, subJobId, mainLevel, subLevel)
    local mainJob = data.jobName(mainJobId);
    local subJob = data.jobName(subJobId);

    return {
        name = name,
        mainJob = mainJob,
        subJob = subJob,
        mainJobLevel = mainJob ~= data.unknownJob and mainLevel or '',
        subJobLevel = subJob ~= data.unknownJob and subLevel or '',
        zone = data.zoneName(zoneId),
        zoneId = zoneId,
    };
end
